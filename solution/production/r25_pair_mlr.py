"""[m35 = m32 + MLRFEAT hook: family-specific mechanism log-likelihood-ratio features (s53)] Pair model v6: interaction + surprisal + suppressed-action + MIL hand aggregates; PU cleaning of hidden positives in dev U."""
...
def load(name):
    ...
    if os.environ.get("MLRFEAT", "0") == "1":
        isf = pd.read_parquet(f"{OUT}/s53_mlr_{name}.parquet").set_index("slot")
        for c in isf.columns: t["isf_" + c] = t.slot.map(isf[c]).fillna(0).values
...
def feats(t):
    ...
    if os.environ.get("MLRFEAT", "0") == "1":
        parts.append(t[[c for c in t.columns if c.startswith("isf_")]].astype(np.float32).reset_index(drop=True))
