"""[m34 = m32 + INFOFEAT hook: partner-private-info dependence features] Pair model v6: interaction + surprisal + suppressed-action + MIL hand aggregates; PU cleaning of hidden positives in dev U."""
...
def load(name):
    ...
    if os.environ.get("INFOFEAT", "0") == "1":
        isf = pd.read_parquet(f"{OUT}/s23_infoshare_{name}.parquet").set_index("slot")
