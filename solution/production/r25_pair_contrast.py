...
CONTRAST = os.environ.get("CONTRAST", "1") == "1"
if CONTRAST:
    ccols = [c for c in X["eval"].columns if (c.endswith("_c_max") or c.endswith("_z_max") or c.endswith("_cr_max") or c.endswith("_spec_max") or c.startswith("mil_")) and not c.startswith("pl")]
    for nm in ["devsub11", "devsub12", "dev", "eval"]:
        assert (tabs[nm].key.values == tabs["eval"].key.values).all()
    other = {"devsub11": X["eval"], "devsub12": X["eval"], "dev": X["eval"], "eval": (X["devsub11"][ccols] + X["devsub12"][ccols]) / 2}
    for nm in ["devsub11", "devsub12", "dev", "eval"]:
        dlt = (X[nm][ccols].values - other[nm][ccols].values).astype(np.float32)
        X[nm] = pd.concat([X[nm], pd.DataFrame(dlt, columns=["d_" + c for c in ccols])], axis=1)
    log("contrast features", len(ccols))
