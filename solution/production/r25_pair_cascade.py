...
# ---------------- cascade: stage-2 re-ranker on the stage-1 top region (per subsample), fold-safe
KTOP = int(os.environ.get("KTOP", "4000"))
T["s1"] = oof
XT2 = XT.copy(); XT2["s1_logit"] = np.log(np.clip(oof, 1e-7, 1 - 1e-7) / (1 - np.clip(oof, 1e-7, 1 - 1e-7)))
top = np.zeros(len(T), bool)
for nm in ["devsub11", "devsub12", "eval_lab"]:
    m = (T.src == nm).values
    if m.sum() == 0: continue
    r = pd.Series(oof[m]).rank(ascending=False, method="first").values
    top[np.where(m)[0][r <= KTOP]] = True
p2 = dict(params, num_leaves=15, min_data_in_leaf=20, learning_rate=0.02, feature_fraction=0.5)
oof2 = oof.copy()
for f in range(5):
    tr = top & (T.fold.values != f) & (w > 0); va = top & (T.fold.values == f)
    m2 = lgb.train(p2, lgb.Dataset(XT2[tr], y[tr], weight=w[tr]), num_boost_round=int(os.environ.get("R2", "600")))
    pr = m2.predict(XT2[va])
    oof2[va] = 1.0 + pr   # keep the top region above the rest, reorder inside it
for nm in ["devsub11", "devsub12"]:
    mm = (T.src == nm).values; mc = mm & ~T.hid.values
    log(f"CASCADE K={KTOP} {nm}: AP_raw {average_precision_score(yl[mm], oof2[mm]):.4f} (stage1 {average_precision_score(yl[mm], oof[mm]):.4f})  AP_clean {average_precision_score(yl[mc], oof2[mc]):.4f} (stage1 {average_precision_score(yl[mc], oof[mc]):.4f})")
