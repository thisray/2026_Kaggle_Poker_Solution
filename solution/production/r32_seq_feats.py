"""R3-E17: the t52 evidence features for EVERY co-seated hand of the dev positive pairs (t5_dev_seq rows),
so the R18-style censored event model can use them for DT and SP. Expressions copied verbatim from t52."""
import numpy as np, pandas as pd, os
from pathlib import Path
OUT = os.environ["POKER_WORK_DIR"]; RAW = os.environ["POKER_DATA_DIR"]; D = f"{OUT}/np"
Path(OUT, "r3").mkdir(parents=True, exist_ok=True)
import pairindex as PI
seq = pd.read_parquet(f"{OUT}/t5_dev_seq.parquet").rename(columns={"sl": "slot"})[["slot", "h"]].drop_duplicates().reset_index(drop=True)
...
d = seq.merge(lab[["slot", "pa", "pb"]], on="slot", how="inner")
...
d.to_parquet(f"{OUT}/r3/t58_seq_feats.parquet"); print("saved", d.shape)
