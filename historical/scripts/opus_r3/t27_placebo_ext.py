...
sets = {"eval_mid_bf2": B[(B.member != True) & (~B.pair_id.isin(b6)) & (B.rk_r2j2m > 600) & (B.bf_hier > 2)].slot.values,
        "eval_top600_bf0to5": B[(B.member != True) & (~B.pair_id.isin(b6)) & (B.rk_r2j2m <= 600) & (B.bf_hier > 0) & (B.bf_hier <= 5)].slot.values}
...
R.to_parquet(f"{R3}/t27_placebo_ext.parquet")
pd.set_option("display.width", 220)
print(R.groupby("set")[["bfP", "bfO", "D"]].describe(percentiles=[.1, .25, .5, .75, .9]).round(1).T.to_string())
R["Dn"] = R.D / np.sqrt(R.n)
print(R.sort_values("Dn", ascending=False).head(30)[["set", "pair_id", "rk_r2j2m", "predicted_behavior", "bf_hier", "bfP", "bfO", "D", "n", "Dn"]].round(2).to_string(index=False))
