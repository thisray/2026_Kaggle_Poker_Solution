...
lo = o.key // 12000; hi = o.key % 12000; lp = pd.read_parquet(f"{OUT}/player_local_v1.parquet").set_index("player_gi"); o["slot"] = lp.pool.loc[lo].values * 900 + lp.local.loc[lo].values * 30 + lp.local.loc[hi].values
tail = o[(o.rk > 300) & (o.rk <= 5000)]
sets = {"tail_pos_" + f[:2]: g.slot.values for f, g in tail[tail.y == 1].groupby("fam")}
sets["tail_neg"] = tail[(tail.y == 0) & (~tail.hid)].sample(400, random_state=1).slot.values
...
R = pd.concat(out, ignore_index=True).merge(o[["slot", "rk", "oof", "y", "fam"]], on="slot", how="left")
R.to_parquet(f"{R3}/t31_bfo_tail.parquet")
pd.set_option("display.width", 220)
print(R.groupby("set")[["bfP", "bfO", "D"]].describe(percentiles=[.1, .25, .5, .75, .9]).round(1).T.to_string())
R["Dn"] = R.D / np.sqrt(R.n)
print(R.groupby("set")[["bfP", "bfO", "D", "n"]].median().round(2).to_string())
from sklearn.metrics import roc_auc_score
T = R[R.set.str.startswith("tail_pos_") | (R.set == "tail_neg")].copy(); T["yy"] = (T.set != "tail_neg").astype(int)
for c in ["bfO", "bfP", "D", "oof"]:
    print(f"AUC tail positives vs negatives by {c}: {roc_auc_score(T.yy, T[c]):.3f}")
for f in ["tail_pos_di", "tail_pos_so", "tail_pos_co"]:
    U = T[(T.set == f) | (T.set == "tail_neg")]
    if U.yy.sum() > 3: print(f, "n", int(U.yy.sum()), "AUC bfO", round(roc_auc_score(U.yy, U.bfO), 3), "AUC oof", round(roc_auc_score(U.yy, U.oof), 3))
