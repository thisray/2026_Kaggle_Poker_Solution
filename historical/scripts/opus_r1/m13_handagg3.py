...
import handfeat3 as HF2, pairindex as PI, aggmod as AG
OUT = HF2.OUT
t0 = time.time()
def log(*a): print(f"[{time.time()-t0:7.1f}s]", *a, flush=True)
dv = pd.read_parquet(f"{OUT}/m1_dev_oof.parquet")
fold_of_pool = dv.groupby("pool").fold.first().reindex(range(400)).values
models = [lgb.Booster(model_file=f"{OUT}/m12_hand_f{f}.txt") for f in range(5)]
_hs = pd.read_parquet(f"{OUT}/m12_handscores.parquet"); _neg = _hs[(_hs.phase == 0) & (~_hs.pos)].s.values; q99, q999 = np.quantile(_neg, [0.99, 0.999]); log("thresholds", q99, q999)
__import__("json").dump({"q99": float(q99), "q999": float(q999)}, open(f"{OUT}/m13_thresholds.json", "w"))
res = {}
for phase in [0, 1]:
    ...
    for i in range(0, len(H), B):
        X = HF2.features(H[i:i+B], S[i:i+B], T[i:i+B])
        if phase == 0:
            fo = fold_of_pool[SL[i:i+B] // 900]
            out = np.zeros(len(X), np.float32)
            for f in range(5):
                m = fo == f
                if m.any(): out[m] = models[f].predict(X[m], num_threads=18)
            sc[i:i+B] = out
        else:
            sc[i:i+B] = np.mean([m.predict(X, num_threads=18) for m in models], axis=0)
        log("phase", phase, "scored", i + len(X))
    res[phase] = (H, SL, sc)
    np.save(f"{OUT}/m13_handscore_phase{phase}.npy", sc)
    np.save(f"{OUT}/m13_slot_phase{phase}.npy", SL); np.save(f"{OUT}/m13_h_phase{phase}.npy", H)
log("scored all")
def aggregate(H, SL, sc, handmask, name):
    ...
    agg = agg.reset_index(); agg.to_parquet(f"{OUT}/m13_mil_{name}.parquet"); log("agg", name, agg.shape)
