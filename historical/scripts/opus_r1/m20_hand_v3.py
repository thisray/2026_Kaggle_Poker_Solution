...
import handfeat as HF, handfeat4 as HF2, pairindex as PI
...
if os.environ.get("DESC", "1") == "1":
    X = pd.concat([X, HD.descriptors(h, s, t, os.environ.get('PROBS', 'dec_probs_v2.npy'))], axis=1)
...
ts_all = np.load(f"{OUT}/np/h_ts.npy"); tsr = ts_all[h]
_df = pd.DataFrame({"sl": sl, "ts": tsr, "ev": is_ev, "pb": pos_bag})
_last = _df[_df.ev].groupby("sl").ts.max()
before_last = pos_bag & (~is_ev) & (tsr < pd.Series(sl).map(_last).fillna(-1).values)
HN = os.environ.get("HN", "1") == "1"
if HN:
    neg = neg | before_last
log("hard negatives (non-ev before last ev) added:", int(before_last.sum()), "HN", HN)
params = dict(objective="binary", learning_rate=0.05, num_leaves=63, min_data_in_leaf=40, feature_fraction=0.6, bagging_fraction=0.8, bagging_freq=1, lambda_l2=2.0, verbose=-1, num_threads=12, seed=5)
y = is_ev.astype(int); trainable = is_ev | neg
oof = np.zeros(len(y))
for f in range(5):
    tr = trainable & (fold != f)
    w = np.where(y[tr] == 1, 1.0, y[tr].sum() / (len(y[tr]) - y[tr].sum()) * 3)
    w = np.where(before_last[tr], w * float(os.environ.get("HNW", "1")), w)
    mdl = lgb.train(params, lgb.Dataset(X[tr], y[tr], weight=w), num_boost_round=600); mdl.save_model(f"{OUT}/" + os.environ.get("MTAG", "m17") + f"_hand_f{f}.txt")
...
D.to_parquet(f"{OUT}/" + os.environ.get("MTAG", "m17") + "_handscores.parquet")
