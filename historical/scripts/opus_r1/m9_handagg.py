...
import handfeat2 as HF2, pairindex as PI, aggmod as AG
OUT = HF2.OUT
t0 = time.time()
def log(*a): print(f"[{time.time()-t0:7.1f}s]", *a, flush=True)
dv = pd.read_parquet(f"{OUT}/m1_dev_oof.parquet")
fold_of_pool = dv.groupby("pool").fold.first().reindex(range(400)).values
models = [lgb.Booster(model_file=f"{OUT}/m6_hand_f{f}.txt") for f in range(5)]
meta = __import__("json").load(open(f"{OUT}/m8_evrank_meta.json")); q99, q999 = meta["q99"], meta["q999"]
res = {}
for phase in [0, 1]:
    ...
    res[phase] = (H, SL, sc)
    np.save(f"{OUT}/m9_handscore_phase{phase}.npy", sc)
    np.save(f"{OUT}/m9_slot_phase{phase}.npy", SL); np.save(f"{OUT}/m9_h_phase{phase}.npy", H)
log("scored all")
def aggregate(H, SL, sc, handmask, name):
    ...
    agg = agg.reset_index(); agg.to_parquet(f"{OUT}/m9_mil_{name}.parquet"); log("agg", name, agg.shape)
