...
import handfeat as HF, handfeat3 as HF2, pairindex as PI
...
for f in range(5):
    ...
    mdl = lgb.train(params, lgb.Dataset(X[tr], y[tr], weight=w), num_boost_round=600); mdl.save_model(f"{OUT}/m12_hand_f{f}.txt")
...
D.to_parquet(f"{OUT}/m12_handscores.parquet")
