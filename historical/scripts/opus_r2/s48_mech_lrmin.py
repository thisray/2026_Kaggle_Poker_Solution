"""[s48: MIN over directions] Neyman-Pearson pair statistic for the fourth family: log-likelihood ratio of the fitted three-action mixture model
...
for ph in [0, 1]:
    ...
    out[ph] = np.minimum(ACC[:, 0, 0], ACC[:, 1, 0])          # symmetric requirement: min over directions
...
e[["slot", "pair_id", "rk", "L", "mem"]].to_parquet(f"{OUT}/s48_mech_llrmin_eval.parquet")
