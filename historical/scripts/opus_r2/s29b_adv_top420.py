...
te = te.merge(ev[["slot", "pair_id", "rk"]], on="slot"); etop = te[te.rk <= 420].copy()
old = pd.read_parquet(f"{OUT}/s23_infoshare_eval.parquet"); old["p2"] = np.minimum(old.za0 - old.zf0, old.za1 - old.zf1); etop = etop.merge(old[["slot", "p2"]], on="slot", how="left"); etop = etop[~(etop.p2 > 2.5)].reset_index(drop=True)
...
etop[["slot", "pair_id", "rk", "p2", "adv"]].to_parquet(f"{OUT}/s29b_adv_eval_top420.parquet")
