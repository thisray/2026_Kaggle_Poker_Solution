...
old = pd.read_parquet(f"{OUT}/s78_f4_pair_alpha.parquet")[["slot", "h", "P1", "P2"]]
j = Hh.merge(old, on=["slot", "h"], how="inner"); print("corr(hand posterior, first-decision P1):", round(np.corrcoef(j.post, j.P1)[0, 1], 3), " n", len(j))
