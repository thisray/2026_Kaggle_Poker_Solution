...
# ---- narrow table export for handoff (ChatGPT round-6 request)
hidx = pd.read_parquet(f"{OUT}/np/hand_index.parquet").sort_values("hi")
hand_ids = hidx.hand_id.values
pidx = pd.read_parquet(f"{OUT}/np/player_index.parquet")
pi2pid = dict(zip(pidx.pi, pidx.player_id))
imp = None
...
keep = [c for c in imp.sort_values(ascending=False).index if c in X.columns][:64]
extra = ["DS_pair_fold_to_partner", "DS_pair_calls_partner", "DS_iso_fold", "DS_o_fold_after_pair",
         "ev_partner_concession", "ev_outsider_fold_after_pair_pressure", "wit_r2c_max", "wit_c2f_sum"]
cols_export = list(dict.fromkeys(keep + [c for c in extra if c in X.columns]))
tt = pd.read_parquet(f"{OUT}/m25t1_handfeat2_m19w10_oof.parquet").sort_values(["sl", "ts"]).reset_index(drop=True)
s1v = P.s.values
m_p = P.groupby("sl").ev.sum()
rank = pd.Series(P.u_r5b.values).groupby(sl).rank(ascending=False, method="first")
top5_flag = (rank.values <= 5)
h_p = pd.Series(np.where(top5_flag, P.ev.values.astype(int), 0)).groupby(sl).sum()
T = pd.DataFrame({
    "slot": sl, "pool": sl // 900,
    "pair_player_lo": [pi2pid.get(x, -1) for x in plo], "pair_player_hi": [pi2pid.get(x, -1) for x in phi],
    "hand_id": hand_ids[h], "fold": fold, "ev": y,
    "m_p": m_p.reindex(sl).values, "top5_pick": top5_flag.astype(int), "h_p": h_p.reindex(sl).values,
    "rank_u_r5b": rank.values,
    "sc_r5": P.sc_r5.values, "u0": P.u_r5.values, "u_rr": P.u_rr.values, "u_r5b": P.u_r5b.values,
    "lin_contrib": (P.u_rr.values - P.u_r5.values), "nn_contrib": (P.u_r5b.values - P.u_rr.values),
    "t1_score": T.sc_fam.values, "s1_stage1": s1v,
})
for c in cols_export:
    T[c] = X[c].values
narrow = T[T.rank_u_r5b <= 20].copy()
narrow.to_parquet(f"{DST}/r6_narrow_candidates.parquet")
narrow.to_csv(f"{DST}/r6_narrow_candidates.csv", index=False)
print("narrow rows", len(narrow), "cols", narrow.shape[1])
print("label rates by rank:", narrow.groupby(narrow.rank_u_r5b.astype(int)).ev.mean().round(3).to_dict())
