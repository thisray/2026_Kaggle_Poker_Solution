...
# ---- subtype gate features: DT concession, SP faced-pressure, CI isolation
imp = None
...
cand_ok = [c for c in imp.sort_values(ascending=False).index if c in X.columns and c.startswith(("z_", "ev_", "wit_", "o_", "DS_", "S_", "Q_"))]
top8 = cand_ok[:8]
Zg = X[top8].values.astype(np.float64)
DT_GATE, SP_GATE, CI_GATE = "DS_pair_fold_to_partner", "DS_pair_calls_partner", "DS_iso_fold"
g = np.zeros(len(P), np.float32)
for k, fm in enumerate(FAMS):
    mask_f = (P.fam.values == fm)
    col = [DT_GATE, SP_GATE, CI_GATE][k]
    g[mask_f] = (X[col].values[mask_f] > 0).astype(np.float32)
CTXBLOCK = np.concatenate([Zg * g[:, None], Zg * (1 - g)[:, None], g[:, None]], axis=1)
print("gate block", CTXBLOCK.shape, "gate mean", float(g.mean()), flush=True)
