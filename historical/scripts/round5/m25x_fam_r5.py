"""Round-4 experiment: family-specific within-pair rerankers vs joint model (same template features as t1)."""
import importlib
import json
import os
import time
...
TPOW = 2.0
RUNTAG = os.environ.get("RUNTAG", "famsep")
t0 = time.time()
...
USE_R5 = os.environ.get("R5", "1") == "1"
if USE_R5:
    E5 = R5.features(h, sa, sb)
    X = pd.concat([X, E5], axis=1)
    print("r5 features added", E5.shape, flush=True)
...
params = dict(objective="binary", learning_rate=0.03, num_leaves=15, min_data_in_leaf=40,
              ...
              verbose=-1, num_threads=12, seed=int(os.environ.get("SEED", "4")))
cols = list(X.columns)
...
def decode(df, col):
    ...
    y = df[col] * poisson.cdf(3, cum) * np.exp(-0.25 * df.groupby("sl").ts.rank(pct=True))
    return df.assign(dec=y)
...
P["sc_joint"] = oof_joint
print("joint:", map5(decode(P, "sc_joint"), "dec")[0], map5(decode(P, "sc_joint"), "dec")[1], flush=True)
...
for fi, fm in enumerate(FAMS):
    ...
    r = map5(decode(P[mask_f], "sc_x") if False else decode(P.assign(sc_x=oof_f)[mask_f], "sc_x"), "dec")
    print(f"family-only {fm}: {r[0]} {r[1]}", flush=True)
P["sc_famsep"] = oof_fam
r2 = map5(decode(P, "sc_famsep"), "dec")
print("family-separated overall:", r2[0], r2[1])
json.dump({"joint": map5(decode(P, "sc_joint"), "dec"), "famsep": r2},
