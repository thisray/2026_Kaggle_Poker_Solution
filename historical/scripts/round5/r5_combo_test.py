"""Round-5 combo test: R5-base + additive t1/nn views + rerank, across seeds."""
import importlib
import json
import os
import time
...
P["u0"] = np.log(np.clip(P.sc * poisson.cdf(3, cum) * np.exp(-0.25 * P.groupby("sl").ts.rank(pct=True)), 1e-9, None))
...
t1 = lg(T.sc_fam.values)
nn = lg(S.nn_cal.values)

# rerank beta trained per fold on R5-base candidates
...
res = {"seed": SEED, "base_R5": eval_rank(P.u0.values)}
for a in [0.1, 0.15]:
    res[f"t1_{a}"] = eval_rank(P.u0.values + a * t1)
    res[f"nn_{a}"] = eval_rank(P.u0.values + a * nn)
res["rerank0.5"] = eval_rank(P.u0.values + 0.5 * lin)
for a in [0.1, 0.15]:
    res[f"rerank0.5_t1_{a}"] = eval_rank(P.u0.values + 0.5 * lin + a * t1)
    res[f"rerank0.5_nn_{a}"] = eval_rank(P.u0.values + 0.5 * lin + a * nn)
res["rerank0.5_t1_0.1_nn_0.15"] = eval_rank(P.u0.values + 0.5 * lin + 0.1 * t1 + 0.15 * nn)
print(json.dumps(res, indent=1))
json.dump(res, open(f"{OUT}/r5_combo_seed{SEED}.json", "w"), indent=2)
