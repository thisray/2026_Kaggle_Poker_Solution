...
def load_pack(pack, entry=None):
    pack = Path(pack); d = pd.read_csv(pack / 'meta.csv')
    if entry:
        e = pd.read_csv(entry)
        d = d.merge(e, on=['slot', 'hand_id'], how='left', validate='one_to_one')
        fill = [c for c in e.columns if c not in ('slot', 'hand_id')]
        d[fill] = d[fill].fillna(0.0)
...
ENTRY = ["en_surprise_sum", "en_surprise_min", "en_both_entered", "en_second_surprise",
         "en_resid_product", "en_expected_both", "en_second_entered", "en_first_entered"]


def entry_block(d):
    cols = [c for c in ENTRY if c in d]
    if not cols:
        return None
    return np.nan_to_num(d[cols].to_numpy(float), nan=0., posinf=0., neginf=0.).astype(np.float32)


def build_x(d, moments, mu, sd):
    raw = dense(d, SCORES)
    parts = [moments, raw, consistency(d, raw, mu, sd)]
    eb = entry_block(d)
    if eb is not None:
        parts.append(eb)
...
def fit(pack, out, seed=71, rounds=500, cat_rounds=400, threads=4, cv_only=False, entry=None):
    out = Path(out)
    if out.exists() and any(out.iterdir()): raise FileExistsError(out)
    out.mkdir(parents=True, exist_ok=True)
    d, mom = load_pack(pack, entry)
...
def main():
    ...
    f.add_argument('--cv-only', action='store_true'); f.add_argument('--entry')
    r = s.add_parser('predict'); r.add_argument('--pack', required=True); r.add_argument('--model', required=True); r.add_argument('--out', required=True); r.add_argument('--entry')
    a = p.parse_args(); args = vars(a); cmd = args.pop('cmd')
    if cmd == 'fit': print(json.dumps(fit(**args), indent=2))
    else:
        d, m = load_pack(a.pack, a.entry); cat, lr, blend = predict_one(d, m, a.model)
