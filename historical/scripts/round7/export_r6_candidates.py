...
t1models = [lgb.Booster(model_file=f"{OUT}/m25t1_handfeat2_fam_m19w10_f{f}.txt") for f in range(5)]
...
while i0 < len(C):
    ...
    ts = ts_all[h]
    t1c = [c for c in all_cols if not (c.startswith("ev_") or c.startswith("wit_"))]
    assert len(t1c) == 468
    t1sc = np.mean([m.predict(X[t1c], num_threads=12) for m in t1models], axis=0)
    D = pd.DataFrame({"slot": sl, "h": h, "s1": s1, "fi": fi_arr, "sa": sa, "sb": sb, "sc": sc, "t1": t1sc, "ts": ts})
    ...
    out_tables.append(cand[["slot", "h", "s1", "fi", "sa", "sb", "u0", "lin", "u", "gen_rank_all", "sc", "t1"]])
