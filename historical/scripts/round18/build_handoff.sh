...
ZIP_PATH="${DOWNLOAD_DIR}/Poker_Round18_Handoff_20260919.zip"
REPORT_PATH="${REPO_ROOT}/docs/37_breakthrough_research_20260919.md"
RECEIPT_PATH="${REPO_ROOT}/receipts/round18_breakthrough_research_20260919.json"
TMP_DIR="$(mktemp -d /tmp/poker-round18-handoff.XXXXXX)"
...
mkdir -p "${STAGE_DIR}/docs" "${STAGE_DIR}/scripts/round18" "${STAGE_DIR}/receipts"

cp "${REPO_ROOT}/README.md" "${STAGE_DIR}/README.md"
cp "${REPORT_PATH}" "${STAGE_DIR}/00_STATE_SNAPSHOT_ROUND18.md"
...
find "${REPO_ROOT}/scripts/round18" -type f -print0 | while IFS= read -r -d '' source_file; do
  relative_path="${source_file#${REPO_ROOT}/scripts/round18/}"
  mkdir -p "${STAGE_DIR}/scripts/round18/$(dirname "${relative_path}")"
  cp "${source_file}" "${STAGE_DIR}/scripts/round18/${relative_path}"
done
...
perl -pi -e 's/^- ZIP SHA-256：.*$/- ZIP SHA-256：由 repository receipt 記錄（ZIP 內不嵌入自我雜湊）。/' \
  "${STAGE_DIR}/00_STATE_SNAPSHOT_ROUND18.md" \
  "${STAGE_DIR}/docs/37_breakthrough_research_20260919.md"
perl -pi -e 's/"sha256": "PENDING_HANDOFF_BUILD"/"sha256": "RECORDED_IN_REPOSITORY_RECEIPT"/' \
  "${STAGE_DIR}/receipts/round18_breakthrough_research_20260919.json"
