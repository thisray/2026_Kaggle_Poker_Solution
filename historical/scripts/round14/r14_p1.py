...
pe = pd.read_parquet(S / "r14_pexp_v2" / "pair_entry_expected_v2.parquet").rename(columns={"plo": "p_lo", "phi": "p_hi"})
...
res["pexp_v2"] = train_eval(XT, "v6pexpv2")
json.dump(res, open(OUT / "r12_p1.json", "w"), indent=2)
out = T[["key", "pool", "fold", "src", "y", "label", "n", "hid"]].copy()
out["base_score"] = _oof_store["v6base"]; out["new_score"] = _oof_store["v6pexpv2"]
