...
def run(input,out,threads=2):
    out=Path(out);out.mkdir(parents=True,exist_ok=True)
    d=pd.read_csv(input); assert not d.duplicated(['slot','hand_id']).any()
    assert d.groupby('pool').fold.nunique().max()==1
    config=dict(protocol='frozen_upstream_exploration',seed=20260918,iterations=180,depth=5,learning_rate=.04,l2_leaf_reg=20.,feature_sets=['score_comparison','score_entry_comparison'],decode='antisymmetric_expected_wins',no_outer_weight_search=True)
    ...
    for name,cols in [('score_comparison',SCORES),('score_entry_comparison',SCORES+ENTRY)]:
        ...
        for fold in sorted(np.unique(f)):
            tr=f[aa]!=fold
            m=CatBoostClassifier(iterations=180,depth=5,learning_rate=.04,l2_leaf_reg=20,random_seed=20260918,thread_count=threads,verbose=False,allow_writing_files=False)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--input',required=True);p.add_argument('--out',required=True);p.add_argument('--threads',type=int,default=2);run(**vars(p.parse_args()))
