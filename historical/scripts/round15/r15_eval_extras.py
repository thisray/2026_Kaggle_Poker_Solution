...
sys.path.insert(0, "/home/thisray/projects/260916_Kaggle_Poker_workers/round3-research-20260917")
sys.path.insert(0, "/home/thisray/projects/260916_Kaggle_Poker_workers/opus-r1-20260917")
sys.path.insert(0, "/home/thisray/projects/260916_Kaggle_Poker_workers/round3-research-20260917")
...
OUT = Path("/home/thisray/projects/260916_Kaggle_Poker_artifacts/round15_campaign"); OUT.mkdir(parents=True, exist_ok=True)
t0 = time.time()
...
d = pd.read_csv(S / "gated_candidates.csv")
...
d["h"] = d.h.astype(int)
cache = pd.read_parquet(f"{OP}/subs/r2_m19w10_eval_hands.parquet")[["slot", "h", "s1", "s2"]]
d = d.merge(cache, on=["slot", "h"], how="left", validate="many_to_one")
assert d.s1.notna().all(), int(d.s1.isna().sum())
log("gated rows", d.shape)

stats = np.load(f"{OP}/r5_template_stats.npz", allow_pickle=True)
...
for c in NEED:
    out[c] = X[c].to_numpy()
out.to_csv(OUT / "gated_extras.csv", index=False)
