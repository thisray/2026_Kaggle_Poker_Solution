"""R4-Q1b: pair model v6 trained with ADDITIONAL exposure-matched dev subsamples (env EXTRA_SUBS=13,14,15; tables in r4/ from q1_more_devsub.py).
OOF is still scored on devsub11 / devsub12 only, same pool folds; outputs go to r4/. Patched copy of opus_r3/t69_pair_variant.py.
...
import os as _o0
EXTRA = [f"devsub{x}" for x in _o0.environ.get("EXTRA_SUBS", "").split(",") if x]
def load(name):
    _d = f"{OUT}/r4" if name in EXTRA else OUT
    t = pd.read_parquet(f"{_d}/ptab_{name}.parquet"); t4 = pd.read_parquet(f"{_d}/ptab4_{name}.parquet")
    ...
    if MIL != "none":
        mil = pd.read_parquet(f"{_d}/{MIL}_mil_{name}.parquet").set_index("slot")
...
tabs = {nm: load(nm) for nm in ["devsub11", "devsub12", "dev", "eval"] + EXTRA}; log("loaded")
...
for _e in EXTRA: hidden[_e] = hidden["devsub11"] | hidden["devsub12"]
parts = []; px = []
for nm in ["devsub11", "devsub12"] + EXTRA:
    t = tabs[nm]
...
ev[inev][["key", "pool", "p_lo", "p_hi", "n"]].assign(score=s_eval).to_parquet(f"{OUT}/r4/m15_{TAG}_eval_scores.parquet")
T[["key", "pool", "fold", "src", "y", "fam", "label", "n", "hid"]].assign(oof=oof).to_parquet(f"{OUT}/r4/m15_{TAG}_train_oof.parquet")
