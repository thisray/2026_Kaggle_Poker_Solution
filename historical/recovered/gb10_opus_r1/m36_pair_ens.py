"""[m36 = m32 + SEEDS/LEARNER ensembling (rank-averaged)] Pair model v6: interaction + surprisal + suppressed-action + MIL hand aggregates; PU cleaning of hidden positives in dev U."""
...
SEEDS = [int(s) for s in os.environ.get("SEEDS", "7").split(",")]
LEARNER = os.environ.get("LEARNER", "lgb")      # lgb | cat | both
from scipy.stats import rankdata
def fit_predict(Xtr, ytr, wtr, Xva_list):
    outs = [np.zeros(len(x)) for x in Xva_list]
    if LEARNER in ("lgb", "both"):
        for sd in SEEDS:
            p = dict(params); p["seed"] = sd
            mdl = lgb.train(p, lgb.Dataset(Xtr, ytr, weight=wtr), num_boost_round=800)
            for i, x in enumerate(Xva_list): outs[i] += rankdata(mdl.predict(x)) / len(x) / len(SEEDS) / (2 if LEARNER == "both" else 1)
    if LEARNER in ("cat", "both"):
        from catboost import CatBoostClassifier
        for sd in SEEDS:
            cb = CatBoostClassifier(iterations=1500, depth=6, learning_rate=0.05, l2_leaf_reg=10, random_seed=sd, thread_count=8, verbose=False, allow_writing_files=False)
            cb.fit(Xtr, ytr, sample_weight=wtr)
            for i, x in enumerate(Xva_list): outs[i] += rankdata(cb.predict_proba(x)[:, 1]) / len(x) / len(SEEDS) / (2 if LEARNER == "both" else 1)
    return outs
oof = np.zeros(len(T))
for f in range(5):
    tr = (T.fold.values != f) & (w > 0); va = T.fold.values == f
    oof[va] = fit_predict(XT[tr], y[tr], w[tr], [XT[va]])[0]
yl = T.y.values
for nm in ["devsub11", "devsub12"]:
...
s_eval = fit_predict(XT[w > 0], y[w > 0], w[w > 0], [X["eval"][inev]])[0]
full = lgb.train(params, lgb.Dataset(XT[w > 0], y[w > 0], weight=w[w > 0]), num_boost_round=50)   # only for the importance printout
